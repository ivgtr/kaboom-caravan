...
- 勝利までのSimulation戦闘時間は85.717〜86.250秒
- 代表戦略は距離を維持し、RailgunとMachine CannonをResourceに応じて切り替える
- Main／Sub使用率は約54／46%、累積前進距離は549.078〜549.258
- 敵弾は各Run 3発発射、3発命中、回避0としてEventから計測
...
代表戦略の最低HPは79〜91で、Battle 7／8のArtillery弾は全弾命中した。Bossも距離60前後を
維持する戦略へ1発を命中させ、Railgun 13発で完了する。長距離維持がまだ強すぎるかは、
実操作で前進報酬とのトレードオフを含めて判断する。
...
- Bossの攻撃1回が距離60付近のPlayerへ十分な圧力になるか
