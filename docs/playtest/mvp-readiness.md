...
- 勝利までのSimulation戦闘時間は103.867〜121.917秒
- 初期Subは近距離用Scatter Cannonで、報酬からRocket／Railgunへ更新できる
- Main／Sub使用率は約83／17〜85／15%、累積前進距離は657.111〜662.474
- 敵弾は各Run 11〜12発発射され、迎撃パリィは各Run 11回成功
- 迎撃パリィを`F`へ割り当て、接触攻撃・敵弾の無効化、反撃、Energy回復、排熱、Cooldown即時回復を確認
- 副武器を`C`、Energyを使う保持式の急加速を左`Shift`へ割り当て、戦闘操作を役割別に分離
...
代表戦略の最低HPは95〜100で、全Seedが完走する。自動戦略は攻撃到達を予測して
パリィするため、手動操作では受付0.42秒の読みやすさと、失敗時の圧力を確認する。
Bossは13.35〜13.42秒、Battle 9は11.9〜20.1秒で、武器強化後もBossだけが突出するHP配分にはなっていない。
...
1. UI説明を見ずに前進・後退、主武器、副武器、迎撃、急加速を操作できるか
