...
通常起動は軍団攻城戦です。旧版は `/?legacyCombat` を明示した場合だけ起動します。以下は旧版の説明で、新版の仕様ではありません。旧版の既存セーブは読み替えず、そのまま残します。

![武装したキャラバンとモンスター軍団の戦い](assets/key-visual.png)
...
25mより前で敵に命中させると決死突破ゲージが増加します。25〜44mでは+6、45〜64mでは+9、65m以降では+12（命中による加算は0.75秒に1回まで）。パリィ成功はどの位置でも+25。空撃ち・待機では増えず、散弾や範囲攻撃の同時命中でも加算は1回です。

100%になったら `E` または「決死突破」ボタンで **1戦1回のDEATH RIDE** を発動できます。武器の熱と射撃Cooldownが解消され、4秒間だけ火器Cooldown進行が3.25倍、排熱が5倍になります。その代わり、ハナツノ・バクレツミ・モスモコ・ホウシダケの4体がプレイヤー近距離へ即座に乱入します。

これは危機回避技ではありません。**敵弾は消えず、攻撃予兆も止まらず、前線も回復しません。** 追加4体を4秒で倒し切れなくても敵は消えず、そのまま通常戦闘に残ります。押した瞬間に戦場を自分で悪化させ、その負債を短時間の異常火力で返せるかを賭ける技です。

同じ戦闘では再発動できません。発動後も命中・パリィでゲージ自体は再充填でき、貯めた分は次戦へ持ち越せます。敵も敵弾もない間は発動せず、キーを押したまま満タンになっても自動発動しません。一時停止・武器箱選択中は強化時間も停止します。詳しい狙いと調整基準は [DEATH RIDE / 決死突破](death-ride-breakthrough.md) を参照してください。
...
選んだコアは遠征終了まで変更できません。種類は次戦へ持ち越しますが、連携・展開・反攻弾の一時状態は次戦でリセットされます。武器箱選択・一時停止中は状態と時間を保持します。新しい遠征では未選択に戻ります。詳しい仕様・確認手順は [改造コア](combat-cores.md) を参照してください。
...
DEATH RIDEで守備隊ごと戦場を壊す、砲座で守る、危険なら断念する。期限内に敵部隊・敵弾を全滅させても回収成功になり、空の戦場で待つ必要はありません。時間切れでも通常戦闘は続きます。停止・武器箱選択中は期限も止まります。詳細は [寄り道の戦場拠点](battlefield-objectives.md) を参照してください。
...
- 前線で蓄積し、敵4体追加と引き換えに4秒だけ火器を暴走させるDEATH RIDE
