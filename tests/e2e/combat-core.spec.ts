...
const names = { relay: '連携機関', siege: '展開砲座', counter: '反攻蓄電器' } as const;
test('normal starts have no core, and debug previews require the debug flag', async ({ page }) => {
...
for (const [index, id] of (['relay', 'siege', 'counter'] as const).entries()) {
  test(`${id}: core selection preserves ordinary rewards and takes effect in Battle 2`, async ({ page }) => {
    await page.goto('/?debug&corePreview=choice');
    const dialog = page.getByRole('dialog', { name: 'この遠征の戦い方を決めよう' });
    ...
    await expect(page.getByRole('region', { name: '改造コアの使い方' })).toContainText(names[id]);
  });
}
test('core dialog traps background focus, ignores Escape, and supports arrows plus native Enter', async ({ page }) => {
  await page.goto('/?debug&corePreview=choice');
  ...
  await page.keyboard.press('Escape');
  await expect(dialog).toBeVisible();
  await page.locator('.audio-toggle').evaluate((element) => (element as HTMLElement).focus());
  await expect(first).toBeFocused();
  ...
  await expect(page.locator('.reward-panel')).toContainText('改造コア：展開砲座');
});
test('relay arms the opposite weapon on real fire and both held cancels it', async ({ page }) => {
...
test('siege deploys, movement cancels it, and partial deployment freezes during pause and blur', async ({ page }) => {
...
test('counter parries a real incoming projectile and consumes one empowered volley', async ({ page }) => {
  ...
  }, { intervals: [20] }).toBeGreaterThanOrEqual(110);
...
for (const viewport of [
  { width: 1280, height: 800 }, { width: 390, height: 844 }, { width: 320, height: 568 }, { width: 844, height: 390 }, { width: 568, height: 320 },
]) {
  test.describe(`core layout ${viewport.width}x${viewport.height}`, () => {
    test.use({ viewport, hasTouch: true });
    test('can read and tap the third core, then keeps its combat HUD out of the controls', async ({ page }) => {
      await page.goto('/?debug&corePreview=choice');
      const dialog = page.locator('.core-choice-panel');
      await expect(dialog).toBeVisible();
      const fits = await dialog.evaluate((element) => {
        const r = element.getBoundingClientRect();
        return r.left >= 0 && r.top >= 0 && r.right <= innerWidth && r.bottom <= innerHeight && element.scrollWidth <= element.clientWidth;
      });
      expect(fits).toBe(true);
      if (process.env.CAPTURE_UI_REVIEW) await page.screenshot({path: `test-results/core-choice-${viewport.width}x${viewport.height}.png`});
      await page.locator('[data-core="counter"]').tap();
      await expect(page.locator('.reward-panel')).toContainText('改造コア：反攻蓄電器');
      if (process.env.CAPTURE_UI_REVIEW) await page.screenshot({path: `test-results/core-reward-${viewport.width}x${viewport.height}.png`});
      await page.locator('.reward-module button').first().tap();
      await expect(page.locator('.core-hud')).toBeVisible();
      const obstruction = await page.locator('.core-hud').evaluate((element) => {
        const a = element.getBoundingClientRect();
        if (a.left < 0 || a.top < 0 || a.right > innerWidth || a.bottom > innerHeight) return 'viewport';
        const selectors = ['.control-button', '.pause-toggle', '.compact-hud', '.run-hud', '.enemy-chip', '.treasure-chip', '.breakthrough-panel'];
        for (const selector of selectors) for (const other of document.querySelectorAll(selector)) {
          const b = other.getBoundingClientRect();
          if (Math.min(a.right, b.right) > Math.max(a.left, b.left) && Math.min(a.bottom, b.bottom) > Math.max(a.top, b.top)) return selector;
        }
        return null;
      });
      expect(obstruction).toBeNull();
      if (process.env.CAPTURE_UI_REVIEW) await page.screenshot({path: `test-results/core-combat-${viewport.width}x${viewport.height}.png`});
      await page.getByRole('button', { name: '一時停止と操作ガイド' }).tap();
      await expect(page.getByRole('region', { name: '改造コアの使い方' })).toContainText('反攻蓄電器');
