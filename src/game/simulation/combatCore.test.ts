...
function combat(id?: CombatCoreId) {
  ...
  state.enemies = [
    { ...createEnemy('heavy', 'dummy', 110), speed: 0, hitPoints: 10000 },
  ];
  return state;
}
function incoming(id = 'shot') {
  return {
    id,
    ownerId: 'dummy',
    previousPosition: 41,
...
function step(state: SimulationState, command: PlayerCommand = IDLE_COMMAND) {
  return stepSimulation(state, command, dt);
}
function triggers(state: SimulationState) {
  return state.events.filter(({ type }) => type === 'core-triggered');
}
...
describe('relay core', () => {
  it('starts with an ordinary volley, then buffs all pellets and vents the OTHER weapon once', () => {
    ...
    const pellets = second.projectiles.filter(
      ({ weaponId }) => weaponId === 'scatter-cannon',
    );
    expect(pellets).toHaveLength(5);
    expect(
      pellets.every(({ damage }) => Math.abs(damage - 6 * 1.6) < 1e-9),
    ).toBe(true);
    expect(second.player.weaponHeat.primary.heat).toBeCloseTo(
      70 - 12 * dt - 18,
    );
  ...
  it.each(['ready', 'cooldown', 'overheated', 'empty'] as const)(
    'both held cancels the chain even when secondary is %s',
    (reason) => {
      ...
      if (reason === 'overheated')
        state.player.weaponHeat.secondary = { heat: 90, overheated: true };
...
describe('siege core', () => {
  it('deploys on step 90, grants damage and heat bonuses, and emits ready only on transition', () => {
    ...
    expect(
      result.events.filter(({ type }) => type === 'core-ready'),
    ).toHaveLength(1);
    expect(
      step(result).events.filter(({ type }) => type === 'core-ready'),
    ).toHaveLength(0);
  });
  it.each([24.999, 10, 0])('cannot deploy at %sm', (position) => {
    let state = createCombatCoreState();
    for (let i = 0; i < 120; i++)
      state = advanceCombatCore('siege', state, IDLE_COMMAND, position, 0, dt);
  ...
  it('allows exactly 25m but resets immediately on movement input, including a blocked boundary', () => {
    ...
    expect(
      advanceCombatCore('siege', core, IDLE_COMMAND, 25, 0, dt).siegeSeconds,
    ).toBe(1.5);
  ...
  it('requires the actual vehicle to stop coasting and removes the bonus below 25m', () => {
    ...
    expect(
      advanceCombatCore('siege', state, IDLE_COMMAND, 40, 0.01, dt)
        .siegeSeconds,
    ).toBe(0);
    expect(
      advanceCombatCore('siege', state, IDLE_COMMAND, 24.99, 0, dt)
        .siegeSeconds,
    ).toBe(0);
  });
...
describe('counter core', () => {
  it.each(['projectile', 'contact'] as const)(
    'a successful %s parry arms AFTER same-step firing; only the next volley is enhanced',
    (kind) => {
      ...
      if (kind === 'projectile') state.enemyProjectiles = [incoming()];
        ...
        state.enemies = [
          {
            ...createEnemy('heavy', 'dummy', 44.5),
            speed: 0,
            hitPoints: 10000,
            attackWindupRemaining: 0.001,
          },
        ];
      ...
      expect(
        parried.events.some(
          (event) =>
            event.type === 'attack-parried' && event.attackKind === kind,
        ),
      ).toBe(true);
      expect(triggers(parried)).toHaveLength(0);
      expect(parried.core.counterSeconds).toBe(3);
      expect(parried.events).toContainEqual({
        type: 'core-ready',
        coreId: 'counter',
      });
      const fired = step(parried, sub);
      expect(triggers(fired)).toEqual([
        { type: 'core-triggered', coreId: 'counter', slot: 'secondary' },
      ]);
  ...
  it('merely pressing parry without a hit does not arm a shot', () => {
    expect(
      step(combat('counter'), { ...IDLE_COMMAND, activateSkill: true }).core
        .counterSeconds,
    ).toBe(0);
  });
  it('caps simultaneous and repeated parries at one charge; recharging refreshes its lifetime', () => {
    ...
    expect(
      result.events.filter(({ type }) => type === 'attack-parried'),
    ).toHaveLength(2);
    expect(result.core.counterSeconds).toBe(3);
    expect(
      chargeCounterCore(
        'counter',
        { ...result.core, counterSeconds: 0.1 },
        true,
      ).counterSeconds,
    ).toBe(3);
  ...
  it.each(['cooldown', 'ammo', 'energy', 'overheat'] as const)(
    'retains charge on failed fire from %s',
    (cause) => {
      const state = combat('counter');
      ...
      if (cause === 'overheat')
        state.player.weaponHeat.primary = { heat: 90, overheated: true };
      const result = step(state, main);
  ...
  it.each([false, true])(
    'both fire: primary gets priority unless it is blocked (%s)',
    (blocked) => {
      const state = combat('counter');
      state.core.counterSeconds = 3;
      state.player.primaryCooldown = blocked ? 1 : 0;
      const result = step(state, both);
      expect(triggers(result)).toEqual([
        {
          type: 'core-triggered',
          coreId: 'counter',
          slot: blocked ? 'secondary' : 'primary',
        },
      ]);
      const pellets = result.projectiles.filter(
        ({ weaponId }) => weaponId === 'scatter-cannon',
      );
      expect(pellets.every(({ damage }) => damage === (blocked ? 15 : 6))).toBe(
        true,
      );
    },
  );
  it('does not mistake a breakthrough bullet clear for a parry', () => {
    ...
    state.enemyProjectiles = [incoming()];
    expect(
      step(state, { ...IDLE_COMMAND, activateBreakthrough: true }).core
        .counterSeconds,
    ).toBe(0);
  });
});
describe('core integration invariants', () => {
  it('multiplies upgraded/module-derived damage once and keeps normal costs and cooldowns', () => {
    ...
    state.core.counterSeconds = 3;
    const definition = deriveWeaponDefinition(
    ...
    expect(result.player.weaponHeat.primary.heat).toBe(
      definition.heatGenerated,
    );
  });
  ...
  it('has no effects on builds without a core, and pure rule functions do not mutate inputs', () => {
    ...
    expect(
      prepareCoreShot(undefined, core, 'primary', false).damageMultiplier,
    ).toBe(1);
    expect(chargeCounterCore(undefined, core, true)).toBe(core);
    expect(
      prepareCoreShot('relay', core, 'primary', false).core.relaySlot,
    ).toBe('primary');
